...
func newNetwork(sm subnet.Manager, extIface *backend.ExternalInterface, port int, nw ip.IP4Net, l *subnet.Lease) (*network, error) {
	...
	// 这里面会打开tun设备，从tun设备里面读取数据，再给后面的UDP
...
// 实际启动backend，这里主要是监听etcd中的子网信息，有event发生的时候调用对应的处理函数
// 处理函数主要负责处理两种事件，子网被添加还有子网被删除，并根据这些事件进行iptabels规则的书写
// 另外UDP模式下还要处理来自tun设备的数据并通过UDP封包
func (n *network) Run(ctx context.Context) {
	...
	// 通过etcd/kube-apiserver监听subnet的信息,并执行相应的操作
	for {
		select {
