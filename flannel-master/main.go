...
// 参数解析可以查看 https://www.ucloud.cn/yun/33085.html
type CmdLineOpts struct {
	// 如果指定etcd作为subnet的后端存储，需要提供如下几个etcd参数
	// 后端存储还可能是kube-apiserver
	...
	// 如果kubeSubnetMgr为true的话，则使用kube-apiserver为subnet的后端存储
	// （其实是将每个node的subnet信息存入到了node的annotations中）
	// 其余三个参数是作为连接kube-apiserver的必要参数
	kubeSubnetMgr          bool
	kubeApiUrl             string
	...
	// 用于跨节点通信的IP地址，是表示公网IP吗？
	publicIP               string

	// subnet是以lease（租约）的形式存储起来的，到期会自动过期。需要定时续租，
	// 该时间表示在lease过期前多少时间续租，单位是min，默认值是60
...
// 如果kubeSubnetManager为true的话，就使用kube-apiserver作为后端存储，这也是默认的存储方式
...
func main() {
	...
	//第一步：创建subnetManager， subnetManager是用于管理子网的
	...
	wg.Add(1)
	go func() {
		// 如果监听到了监听的系统信号，就调用cancel，所有的调用都终止
	...
	// 第二步，创建backend，并用于注册subnet，也就是处理subnet
	...
	// 如果指定了ipMasq则定期同步iptables,也就是如果变更了subnet.env配置文件
	if opts.ipMasq {
